...
usage：
    ...
    ---------------------------------
    P站搜图：
        p站搜图 [关键词] ?[数量] ?[页数=1] ?[r18](不屏蔽R-18)
        示例：
            p站搜图 樱岛麻衣
            p站搜图 樱岛麻衣 5
            p站搜图 樱岛麻衣 5 r18
            p站搜图 樱岛麻衣#1000users 5
        【多个关键词用#分割】
        【默认为 热度排序】
        【注意空格！！】【在线搜索会较慢】【数量可能不符？可能该页数量不够，也可能被R-18屏蔽】
""".strip()
__plugin_des__ = "P站排行榜直接冲，P站搜图跟着冲"
__plugin_cmd__ = ["p站排行 ?[参数] ?[数量] ?[日期]", "p站搜图 [关键词] ?[数量] ?[页数=1] ?[r18](不屏蔽R-18)"]
...
pixiv_keyword = on_command("p站搜图", aliases={"P站搜图"}, priority=5, block=True, rule=to_me())
...
async def _(bot: Bot, event: MessageEvent, arg: Message = CommandArg()):
    ...
    if (n := len(msg)) == 0 or msg[0] == "":
        ...
        if msg[0] not in rank_dict.keys():
...
async def send_image(
    ...
    if isinstance(event, GroupMessageEvent):
        ...
        for title, author, urls in info_list:
            ...
            idx += 1
        mes_list = custom_forward_msg(mes_list, 2854196306)
